...
#        mode = 'train'
        ...
        left_dir = join(self.dir, 'image_2')#左输入图像
        right_dir = join(self.dir, 'image_3')#右输入图像
        left_imgs = list()
        right_imgs = list()
        if mode == 'train':
    ...
    def __getitem__(self, idx):
        ...
        if self.mode != 'test':
###########################################
#            if type(self.disp_imgs) != None:
               data['disp'] = cv2.imread(self.disp_imgs[idx])[:, :, 0]       ####三维数组
...
if __name__ == '__main__':
    ...
    train_dataset = KITTI2015('/root/data/Share data/data_scene_flow/data_scene_flow', mode='train', transform=train_transform)
