...
class PSMNet(nn.Module):
    def __init__(self, max_disp):
        ...
        self.stackedhourglass = StackedHourglass(max_disp)  # costnet*2放入沙漏网络
        self.D = max_disp
        self.__init_params()

    def forward(self, left_img, right_img):
        if type(left_img)!=None:
            ...
            cost_volume = torch.zeros(B, C * 2, self.D // 4, H, W, requires_grad=True).type_as(left_cost)  # [B, 64, D, 1/4H, 1/4W]
            # for i in range(self.D // 4):
            #     cost_volume[:, :, i, :, i:] = cost[:, :, :, i:]

            for i in range(self.D // 4):  # 从0到最大视差把左图和右图特征结合  两边没有匹配的为零
                if i > 0:
    ...
    def __init_params(self):  # 对模型初始化，m代表每一层，初始化weight和bias
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                m.weight.data.normal_(0, math.sqrt(2. / n))  # normal_正态分布初始化
